...
describe('FireRedVAD capture state', () => {
  it('starts after 200 ms and ends after 100 ms of silence', () => {
    ...
    for (let index = 0; index < 15; index += 1) { seconds += 0.01; const update = machine.process(0.1, seconds); if (update) states.push(update.state); }
  ...
  it('does not end during a pause shorter than 100 ms', () => {
    const machine = new FireRedVadStateMachine();
    const states: string[] = [];
    let seconds = 0;
    for (const [probability, frames] of [[0.9, 35], [0.1, 5], [0.9, 20]] as const) {
