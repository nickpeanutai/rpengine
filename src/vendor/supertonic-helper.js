...
import { trimOuterSilence } from '../audio-trim.ts';
...
export class TextToSpeech {
    ...
    async call(text, lang, style, totalStep, speed = 1.05, silenceDuration = 0.3, progressCallback = null) {
        ...
        for (let i = 0; i < textList.length; i++) {
            ...
            const predictedLength = Math.min(wav.length, Math.floor(duration[0] * this.sampleRate));
            const wavChunk = trimOuterSilence(wav.slice(0, predictedLength), this.sampleRate);
            ...
            if (wavCat.length === 0) {
                ...
                wavCat = wavCat.concat(silence, wavChunk);
            }
        }

        const durCat = wavCat.length / this.sampleRate;
...
/**
 * Construct a voice style from already-parsed JSON data.
 */
export function voiceStyleFromData(voiceStyles, verbose = false) {
    if (!Array.isArray(voiceStyles) || voiceStyles.length === 0) {
        throw new Error('At least one voice style is required');
    }

    const bsz = voiceStyles.length;
    const firstStyle = voiceStyles[0];
    ...
    // Fill in the data
    for (let i = 0; i < bsz; i++) {
        const voiceStyle = voiceStyles[i];
...
export async function loadVoiceStyle(voiceStylePaths, verbose = false) {
    const voiceStyles = await Promise.all(voiceStylePaths.map(async path => {
        const response = await fetch(path);
        if (!response.ok) {
            throw new Error(`Failed to load voice style (${response.status}): ${path}`);
        }
        return response.json();
    }));
    return voiceStyleFromData(voiceStyles, verbose);
}
