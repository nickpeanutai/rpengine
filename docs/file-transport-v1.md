...
Snapshot card transfers may omit `targetHash`; RPEngine validates the complete Character Card V2 and returns its computed hash. Patch and reference transfers still require `targetHash`. `voice.capture.start` returns `voice.capture.transcript` only when `returnTranscript` is `true`. Its optional `silenceBehavior` is `"error"` by default; `"restart"` discards an eight-second silent recording window and continues listening under the same request.
