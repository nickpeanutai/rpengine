...
Capture-level telemetry is also lossy in file mode: only the newest `voice.capture.level` sample is retained for each 250 ms window. A request-scoped state, transcript, reply, cancellation, error, or audio event discards any superseded level sample before it is queued, so waveform telemetry cannot delay protocol progress. WebSocket level delivery remains immediate.
...
Snapshot card transfers may omit `targetHash`; RPEngine validates the complete Character Card V2 and returns its computed hash. Patch and reference transfers still require `targetHash`. `voice.capture.start` returns `voice.capture.transcript` only when `returnTranscript` is `true`. Its optional `silenceBehavior` is `"error"` by default; `"restart"` discards an eight-second silent recording window or an empty STT result and continues listening under the same request without returning an error or empty transcript.
